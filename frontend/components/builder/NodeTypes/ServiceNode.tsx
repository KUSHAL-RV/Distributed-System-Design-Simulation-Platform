  ...
  return (
    <div
      className={`
        ...
        ${selected && !isKilled ? 'border-emerald-400 shadow-emerald-500/30 shadow-xl' : 'border-emerald-500/40'}
      `}
    >
      {isKilled && (
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <p className="text-xs font-medium text-emerald-300/70 uppercase tracking-wider">Service</p>
          <p className="text-sm font-semibold text-white">{(data as any).label || 'Service'}</p>
        </div>
      </div>

      {(data as any).baseLatency != null && (
        <div className="mt-2 flex gap-2 text-xs text-emerald-300/60">
          <span>{(data as any).baseLatency}ms latency</span>
          <span>•</span>
          <span>{(data as any).maxConcurrency || '∞'} conc.</span>
        </div>
      )}

      <Handle type="target" position={Position.Top} className="!w-3 !h-3 !bg-emerald-400 !border-2 !border-emerald-900" />
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-emerald-400 !border-2 !border-emerald-900" />
    </div>
  );
}
